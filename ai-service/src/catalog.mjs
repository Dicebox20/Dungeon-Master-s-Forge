const PATTERN_GUIDANCE = Object.freeze({
  ...
  weaponConditionOnHit: "Magic weapon with extraDamageParts and conditionOnHit. conditionOnHit includes conditionName, save ability/DC, and duration seconds. Keep the attack and condition rider together only for this family.",
  ...
  chargedSaveDamage: "One save/damage activity. Use uses, activityId, activationType, chargeCost, save {ability,dc}, damageParts, target, and halfOnSave.",
  ...
  nativeEnchant: "One-use native enchantment. Use activityId, effectId, uses, duration, restrictions, and enchantChanges. Enchantment changes target the selected weapon or armor, not the oil itself.",
  nativeSummon: "One summon profile. Use activityId, profileId, uses, activationType, duration, range, target, and actor with name/type/AC/HP/abilities/movement/traits/items.",
...
Use DND5e v5.3.3-compatible values. Rarity values include common, uncommon, rare, veryRare, legendary, and artifact. Attunement is usually "" or "required". Damage parts use {number, denomination, bonus, types:[damageType]}. Uses use {max, recovery:[{period,type,formula?}], autoDestroy?}. Saves use {ability, dc}. Ranges use {value, long?, units:'ft'}; areas use target.template with type and size. Effect changes use Foundry paths such as system.attributes.ac.bonus, system.bonuses.abilities.save, system.bonuses.msak.attack, system.bonuses.rsak.attack, system.attributes.spelldc, and system.traits.dr.value.

Every activityId, effectId, profileId, toggle-light ID, and unresolved-mechanic ID must be exactly 16 ASCII alphanumeric characters. The service will fill missing IDs, but never invent malformed ones. Separate attacks, healing, saves, utility powers, enchantments, and summons into distinct activities. Existing spells with a saving throw must use saveActivities even when they deal no damage. Unknown spells, ally auras, class-resource restoration, and table-judgment clauses become unresolvedMechanics instead of fake personal effects.
