...
- Candidate archive SHA-256: `9728AAB786FB589E3518BCEE84B35F12C2D42FA3FC97BC104FA534C955B1E90C`
- The current workspace candidate is already installed on disk in Foundry as V2.21.7.
...
- Non-destructive native-content diagnostics now cover Command, Flame Strike, Longsword, Plate Armor, Dire Wolf, Magic Resistance, and Wand of Wonder Effects through system-owned DND5e packs.
- The service workspace now includes `.env.example` for private local configuration with `npm run start:env`.

## Current Release

- Dungeon Master's Forge V2.21.7 is installed on disk in Foundry.
- Installed workspace manifest version: `2.21.7`
- Installed module file count: `36`
- Current installed archive baseline: `outputs/dungeon-masters-forge-v2-2.21.7.zip`
- Current installed archive SHA-256: `9728AAB786FB589E3518BCEE84B35F12C2D42FA3FC97BC104FA534C955B1E90C`
...
- The Items directory launcher and settings shortcut remain visible after the V2.21.3 reload.
...
- V2.21.4 live verification passed for switching Forge Settings from Local Rules to Bring Your Own API, expanding the BYO panel, running Check Connection successfully, and saving the provider so the Forge summary updates to `Connected / Legacy bridge`.
- The first true AI compile exposed a live contract mismatch with an older local bridge on port `8787`; that blocker is now fixed in source by adding client-key OpenAI support to service `1.2.0`.
- A current reference service is now running in live `openai` mode on `http://localhost:8788`, with canonical health and capabilities routes responding and no server-side OpenAI key required for personal client-key testing.
...
- Service 1.2.0 source now supports both server-key OpenAI mode and personal client-key OpenAI mode.
- A current reference service instance is running in `openai` mode on port `8788` for live BYO retesting, while the older legacy bridge still occupies port `8787`.
- Archive: `outputs/dungeon-masters-forge-ai-service-1.2.0.zip`
- Archive SHA-256: `F0150EA2ECE8677077523EAFF161B4C7FE6E230AABD480AE37389AC14D4C9894`
- All 68 source tests and all 68 independently extracted archive tests pass.
- The contract test suite now includes a regression check for `pattern -> kind` normalization.
...
- The current service can either keep `OPENAI_API_KEY` server-side or accept a personal OpenAI key through Foundry's remembered API token field on a trusted computer.
...
- 68 reference-service capability, configuration, contract, prompt, full provider pipeline, adapter, request-integrity and limits, factory-structure, cache, concurrency, remote-content security, compatibility-route, and HTTP checks.
...
4. Replace the former native-first compatibility rule with capability-based routing: select complete reliable DND5e data, verified Midi-QOL/DAE/Item Macro behavior where it supplies the needed workflow, or portable core data plus a review note when the advanced route is unavailable.
