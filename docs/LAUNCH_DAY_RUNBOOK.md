...
This runbook explains how to move a verified Free Forge tester build into the stable Foundry manifest without losing the last known-good release.

## Current Baseline

- Stable module: current local release candidate `2.23.1` (not publishable until the final package review is complete)
- Tester module: latest published tester build `2.23.1-test.45`
- AI service: current hosted service release
- Free Forge allowance: usage-based monthly client and daily global limits; final public values require calibration before launch
- Stable manifest: `module/module.json`
- Tester manifest: `testing/module.json`

## Go or No-Go

Do not promote the tester build until all of these checks pass:
...
- the Droplet usage-ledger backup timer and service restart checks pass
...
Never put the OpenAI key, quota hash secret, or private environment file in GitHub or Foundry settings.
...
If the tester or launch machine exhausts its hosted monthly usage allowance during controlled validation, reset only the current tester bucket on the trusted host:
...
Do not clear daily or monthly usage rows casually during public use; reserve resets for controlled tester validation or recovery after intentionally spent launch checks.

## 4. GitHub Release

1. Preserve the last known-good stable ZIP under `releases/`.
...
- usage-ledger backup completion and disk space
...
1. Restore `module/module.json` to the last known-good stable version and its existing ZIP URL.
...
The Droplet deployment has its own timestamped application, environment, and usage-ledger backups. Restore those only when the service release itself must be rolled back.
