...
- Added the `2.22.0-test.3` tester channel with automatic Free Forge service selection.
- Connected tester installations to the hosted 20-request calendar-month allowance without requiring endpoint or API-token entry.
...
- Added Founding Patron roadmap coverage for optional bespoke item-icon image generation.
- Synced the packaged module contents after the crash so the installed archive, roadmap, and Patreon tier notes match the current workspace.
...
- Kept Hosted Forge disabled pending authentication, entitlement, rate-limit, and abuse-protection work.
